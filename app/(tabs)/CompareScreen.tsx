...
export default function CompareScreen() {
  ...
  // height we want when comparing (use big chunk)
  const compareChartHeight = Math.round(Math.max(320, height * 0.55))
  ...
  return (
    <View style={styles.container}>
      ...
      {/* If comparing, hide the token list and show a big chart */}
      {canCompare ? (
        <View style={styles.compareArea}>
          {/* <View style={styles.compareHeader}>
            <Text style={{ fontWeight: '600' }}>{coinA.toUpperCase()} vs {coinB.toUpperCase()}</Text>
        ...
        <>
          {/* LIST: show only when not comparing */}
          <FlatList
            data={filtered}
            key='one-column'
            numColumns={1}
...
const styles = StyleSheet.create({
  ...
  header: { fontSize: 22, fontWeight: '700', marginBottom: 4, color: '#111' },
  ...
  controlsRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 5 },
  ...
  compareArea: { marginTop: 2 },
  chartWrapper: { backgroundColor: '#fff', borderRadius: 12, padding: 5 },

  hint: { textAlign: 'center', color: '#6b7280', marginTop: 2 },
})
